...
class Elab_model extends BaseModelStrict
{
    ...
    public function getIds($f, $limit, $offset)
    {
        ...
        $sortMap = [
            'no_verifikasi' => 'pr.nomor',
            'tgl_verifikasi' => 'pr.tanggal',
            'nama_pemilik' => 'pr.namaPemilik',
            'jenis_karantina' => 'pr.jenisKarantina',
        ];
    ...
    private function applyManualFilter($f)
    {
        ...
        if (!empty($f['upt_id']) && !in_array(strtolower($f['upt_id']), ['semua', 'all'])) {
            $uptKode = substr($f['upt_id'], 0, 2);
            $this->db->where('pr.uptKode', $uptKode);
        }
        if (!empty($f['lingkup'])) {
            $this->db->where('pr.jenisKegiatan', $f['lingkup']);
        }
        ...
        if (!empty($f['search'])) {
            $this->db->join('`elab-barantin`.penerimaan_detil pd_search', 'pd_search.penerimaanId = pr.id', 'left');
            
            $this->applyGlobalSearch($f['search'], [
                'pr.nomor',
                'pr.namaPemilik',
                'pr.eksDataNomor',
                'pd_search.komoditasUmum',
            ]);
            $this->db->group_by('pr.id');
        }
    }
