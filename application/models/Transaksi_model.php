...
class Transaksi_model extends BaseModelStrict
{
    ...
    private function applyKarantinaJoin(string $karantina, $db = null): bool
    {
        $db = $db ?? $this->db;
        $k = strtolower($karantina);
        if ($k === 'kh' || $k === 'h') {
            $db->join('komoditas_hewan kom', 'pkom.komoditas_id = kom.id');
            $db->join('klasifikasi_hewan klas', 'pkom.klasifikasi_id = klas.id');
            return true;
        } elseif ($k === 'ki' || $k === 'i') {
            $db->join('komoditas_ikan kom', 'pkom.komoditas_id = kom.id');
            $db->join('klasifikasi_ikan klas', 'pkom.klasifikasi_id = klas.id');
            return true;
        } elseif ($k === 'kt' || $k === 't') {
            $db->join('komoditas_tumbuhan kom', 'pkom.komoditas_id = kom.id');
            $db->join('klasifikasi_tumbuhan klas', 'pkom.klasifikasi_id = klas.id');
            return true;
        }
        return false;
    }

    private function buildBaseQuery(array $f, $db = null): void
    {
        $db = $db ?? $this->db;
        ...
        $this->applyKarantinaJoin($f['karantina'] ?? '', $db);
        ...
        if (!empty($f['upt']) && !in_array(strtolower($f['upt']), ['all', 'semua', 'undefined'])) {
            if (strlen($f['upt']) <= 4) {
                ...
                $db->where('p.kode_satpel', $f['upt']);
            }
        }

        if (!empty($f['karantina']) && !in_array(strtolower($f['karantina']), ['all', ''])) {
            $db->where('p.jenis_karantina', substr(strtoupper($f['karantina']), 0, 1));
        }
        ...
        $db->where('p.tgl_dok_permohonan >=', $start . ' 00:00:00');
        $db->where('p.tgl_dok_permohonan <=', $end   . ' 23:59:59');
    }
    ...
    public function getAllForExcel(array $f): array
    {
        ...
        $this->buildBaseQuery($f, $this->db_excel);
